...
`anchor_trusted` on every result reached after the manifest is read. `anneal-memory audit` warns, and
reports `anchor_trusted: false`, when it cannot list the directory to rule out a quarantine and the
manifest is absent.
