...
The authoritative check runs inside the save's DB transaction, after its own write, so a policy change
from another connection either commits first (and is seen) or waits for the save to finish; a failed
check rolls the whole batch back. The baton is a sidecar file outside the transaction, so a take landing
in the milliseconds between that check and the commit is not seen: a documented residual. (A lock held
...
- **New `anneal_memory.sessions` module** — a SIDECAR (not DB) coordination layer extending the lock-sidecar idiom: a TTL'd heartbeat session registry (`<continuity>.sessions/`) + a single consolidate baton (`<continuity>.baton`). Public API: `register_session` / `heartbeat` / `close_session` / `live_sessions` / `claim_baton` / `release_baton` / `baton_holder` / `holds_baton` / `consolidate_authorized`. Keeping coordination state OUT of the SQLite store preserves the single-process/single-writer DB invariant — this layer enforces ONE consolidator; it does not make concurrent DB writes safe.
- **The invariant** (`consolidate_authorized`): a consolidate proceeds iff (sole live session) OR (this session holds the baton); else AUTO-DOWNGRADE to capture-only. A claimed baton OUTRANKS the TTL liveness inference — an idle-but-alive baton-holder that is TTL-reaped does NOT silently cede authority to a parallel lane reading itself "sole" (it downgrades with a distinct `downgraded-stale-baton-holder` reason). `sole-live-session → auto-authorized` means a single-session operator needs ZERO change and never sees the gate.
- **`prepare_wrap(store, *, session_id=None)` — OPT-IN.** The gate engages only when a caller passes `session_id`; every existing caller and single-session adopter is byte-identical (the registry is never consulted, no sidecar created). When engaged and unauthorized it returns the new `status="downgraded"` (`PrepareWrapResult.status` Literal widened `empty|ready` → `empty|ready|downgraded`) leaving the store UNTOUCHED — placed before AM-PREPARE-GUARD and `wrap_started`, so a downgrade never raises, never calls `wrap_cancelled`, and cannot clear another session's in-flight wrap. A consolidate-efferent caller MUST round-trip the returned `wrap_token` to `validated_save_continuity` (the gate throttles WHO starts; the token CAS makes the SAVE safe under a mid-flight baton reclaim — a tokenless save CASes against the current snapshot).
- **New `felt_currency(store) -> FeltCurrency`** — the seal-watermark read: `sealed_at` (last completed consolidate) / `episodes_since_seal` (post-consolidate drift) / `is_current` / `wrap_in_progress`. Surfaces the per-consolidate seal the wrap lifecycle already records, so a stale felt layer (work captured AFTER the day's consolidate) is verifiable, not invisible — no new DB column.
- **Fail-closed.** An unreadable/corrupt FRESH sidecar file is authorization-relevant UNKNOWN (not absent) → `consolidate_authorized` returns `downgraded-registry-error` rather than under-counting a live peer into a false sole-grant (an ABSENT file/dir still reads as empty/unheld — the no-data≠no-event distinction at the collector). SINGLE-MACHINE / local-FS assumption (wall-clock TTL over sidecar mtimes + `os.replace` atomicity); the baton is the structural mitigation for the one wall-clock edge (a forward clock jump > TTL).
- Full 4-layer apparatus: L1 + L2 caught the idle-baton-holder hole (fixed — baton outranks TTL); L3 cross-substrate — **codex (non-replaceable)** caught the CAS-is-hard-only-with-token-round-trip semantic (docstring corrected; the root `wrap_started` cross-connection hardening deferred to a follow-on) + the fail-closed gap + a real `__all__`/import bug; **complement** caught the `session_id=""` API leak; **nemotron**'s structural HIGH/MED claims were refuted against ground truth (don't-average). L4 live end-to-end of the recency-trap journey passes. 38 new tests; full suite green; mypy clean.

### Added — AM-SPORE-CAS: `expect_disposition` optimistic compare-and-set on `SporeStore.ascend`

`ascend` gained an optional `expect_disposition: str | None | _Unset = _UNSET` — the same optimistic CAS `update` already carries, now on the upward-resolve path. A host that enforces a disposition-aware policy in a SEPARATE, lock-free step (e.g. Levain's "a Keep note can't ascend" guard reads the disposition via `get`, then calls `ascend`) passes the raw value it saw; the CAS — checked INSIDE `ascend`'s own transaction (after `_require_open`, before kind-validation + `_resolve`) — aborts with `SporeError("disposition changed since read …")` if a concurrent (cross-process) writer changed it in that read→resolve window, closing the TOCTOU a separate-transaction guard otherwise has. `None` = expect a key-free loop; a string = expect that exact tag. Additive + backward-compatible: omitted (`_UNSET`) skips the CAS, so existing callers are byte-identical. Blind, like every disposition op here — a raw value compare, never an interpretation of the tag. (Surfaced by Levain's KEEP-NOTE LIFECYCLE cross-substrate review: the note-ascend guard's read and `store.ascend` were separate transactions, so a confirmed `loop→note` flip between them could slip a now-note past the guard. Levain's `_apply_spore_verb` now threads the guard-read disposition through this CAS.)

### Added — AM-SPORE-CAS-DESCEND (0.9.5): `expect_disposition` CAS on `SporeStore.descend`

`descend` gained the same optional `expect_disposition: str | None | _Unset = _UNSET` — completing the CAS family across all three disposition-dependent resolve/update verbs (`update` 0.9.1, `ascend` 0.9.4, `descend` 0.9.5). The motivating window is the *downward* mirror of ascend's: a control surface chooses a descend's RESOLVE FACE — Keep "remove" vs Tray "dismiss" vs loop "compost" — off a RENDERED disposition SNAPSHOT, then resolves after a human confirm. Unlike ascend (which reads the disposition server-side for its own `is_note` gate), `descend` has no server-side disposition gate, so the snapshot the FACE was chosen against is the only guard; the caller passes that snapshot and the CAS — checked INSIDE `descend`'s transaction (after `_require_open`, before kind-validation + `_resolve`) — aborts with `SporeError("disposition changed since read …")` if a concurrent writer re-routed the spore in the render→confirm→resolve window, so a now-live loop is not composted under a "remove this note" intent. `None` = expect a key-free loop; a string = expect that exact tag; omitted (`_UNSET`) skips the CAS (byte-identical for existing callers — e.g. a single-flock-atomic CLI). Blind raw-value compare, like the sibling sites. (Surfaced by Levain's spore-170 TUI write-parity review — codex L3 HIGH; fixed under spore-173. Levain's `_apply_spore_verb` descend branch + `_apply_spore_update` tier path now thread the client snapshot; the tier path rides `update`'s existing `expect_disposition`, so it needed no anneal change.)

### Added — AM-RECALL-IDF: corpus-IDF recall weighting + a √N distinctiveness anchor (the precision fix) + per-pattern `source` provenance

`retrieve_relevant`'s keyword weight was a corpus-BLIND length/punctuation proxy (`_keyword_weight`): a term in 30% of the episode corpus (a domain stopword like `work`/`commit`/`project`) scored the same as a genuinely rare term, so any prompt with a couple of process words floated whatever pattern was grounded in an episode it incidentally brushed — the promiscuous-recall failure measured on flow's receipt corpus (a fixed ~6-pattern set surfacing on nearly every prompt, including non-cognitive ones). Replaced on the Store-aware path with a real inverse-document-frequency weight, plus a structural anchor so common-word accumulation can't substitute for a distinctive match.

- **`_idf_weight(df, n) = max(IDF_FLOOR, log((n+1)/(df+1)) / log(n+1))`** — document frequency from the EXACT, uncapped `RecallResult.total_matching` captured in the SAME candidate-fetch calls (zero extra query). Engages only at `corpus_n >= IDF_MIN_CORPUS` (50); below that, and on the Store-free `retrieve_patterns` path, the length-proxy is kept unchanged — so every small-corpus / test / pre-IDF path is byte-identical (the `retrieve_patterns` parity contract holds). `df` is clamped to `corpus_n` (the two counts are separate `recall()`s, not one snapshot; a prune race that would yield `df > n` floors the term instead of going negative — caught by the L3 cross-substrate review).
- **The IDF regime has its own precision bar** `IDF_SCORE_THRESHOLD` (calibrated on flow's real receipt corpus) — separate from the proxy `SCORE_THRESHOLD` because the IDF weight band is lower; selected per-call and threaded through every tier (episodes, keyword-patterns, associative reach) so weights and bar always share one scale.
- **The distinctiveness anchor** `IDF_ANCHOR_WEIGHT` (0.5 on the normalized scale ⟺ df ≤ √N, the geometric-mean document frequency — corpus-size-RELATIVE, needs no per-regime re-calibration): in the IDF regime an item surfaces only if it MATCHED at least one keyword at/above it. This is the STRUCTURAL guard (`structural_invariants_beat_discipline`) against a long process-word-only prompt re-leaking via floor accumulation; the associative tier inherits it through its anchor-gated seeds.
- **`RelevantPattern` gains `source: str = "keyword"`** ∈ `{keyword, evidence_edge, graph_hop}` — the per-pattern provenance a receipt / baseline-isolation consumer needs (keyword = the pattern's own text matched; evidence_edge = a directly keyword-matched evidence episode; graph_hop = reached via one episode-Hebbian hop). Additive (the default keeps old constructors + adopter mocks valid); auto-surfaced in `crystal recall --json` (`asdict`); tie-broken toward the higher-confidence `evidence_edge` on equal reach.
- **⚠ `IDF_SCORE_THRESHOLD` / `IDF_MIN_CORPUS` are flow-CALIBRATED defaults** (a conceptual/partnership corpus regime — much repeated abstract vocabulary). An adopter on an entity-dense regime should re-sweep them against their own receipt corpus; the √N anchor does not drift. Per-call override, a distribution-relative bar, and a single-snapshot stats read are deferred enhancements.
