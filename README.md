...
- ~~**Slow-drift sycophantic graduation across sessions.** A plausible-sounding pattern with plausible-sounding episodes can ride 1x → 2x → 3x across three sessions when each session brings fresh evidence that satisfies lexical overlap. The library has no cross-session "is this pattern actually load-bearing in real downstream decisions" tracker as of the current release.~~ **(partially closed in v0.3.2)** The `pattern_history` table tracks individual prior explanations per pattern across sessions. At graduation time the library compares today's explanation against each prior explanation individually and demotes if the worst-case per-prior overlap is ≥3 meaningful words — the lexical-rephrasing variant of sycophantic accumulation. The deliberately-divergent-vocabulary variant — where each session's explanation uses entirely new words — still passes lexically; that requires the methodology-layer contradiction-scan + operator-review architecture described under "Contradiction with existing graduated patterns" below.
- **Contradiction with existing graduated patterns.** No semantic comparison runs between candidate graduations and existing Proven-tier patterns at the library layer — and structurally cannot, under the no-LLM-as-judge axiom. **The v0.3.2 architectural response splits the responsibility across three layers:** (1) library = audit substrate — `prepare_wrap()` returns `uncovered_proven_to_check: list[str]` (all existing Proven names), `extract_contradiction_declarations()` recognizes per-pattern `[contradicts: name]` / `[no-contradicts]` annotations, `detect_proven_without_declaration()` surfaces new Provens that landed without explicit stance as `proven_without_contradicts_declaration` on the save result and the hash-chained audit-log; (2) methodology layer — the contradiction-scan step at the graduation boundary. **As of v0.4.3 the library emits this step itself**: `prepare_wrap` renders a *Contradiction Scan* block into the agent-facing compression package, listing the existing Proven (`uncovered_proven_to_check`) and the exact `[contradicts: name]` / `[no-contradicts]` marker contract that `detect_proven_without_declaration` parses — so the discipline travels with the wrap instead of depending on an external protocol doc an adopter has to wire and keep in sync; (3) operator-review — an LLM-as-judge contradiction sweep over Proven pairs, surfacing semantic-opposition candidates for partnership review, run on your own cadence; the no-LLM-as-judge axiom keeps this layer out of the zero-dep library by design. The library substrate ships in v0.3.2 and the methodology-scan emission in v0.4.3; the operator-review pass is the layer you run separately. Until all three are in place, "this gap is closed" is overclaim — the library's audit signal records discipline-followed vs not, but external review is what catches the contradictions themselves.
- ~~**Pattern omission.** An adversarial or drift-leaking agent can silently drop previously-graduated patterns by not carrying them forward in the next wrap. The library has no dropout audit across sessions as of the current release.~~ **(closed in v0.3.2)** Proven-tier patterns (level 2x or 3x) absent from the new wrap now surface as `omitted_patterns: [{"name": ..., "prior_level": ...}]` on the `validated_save_continuity` return and ride into the hash-chained audit log under the `continuity_saved` event. This is an audit signal, not a save-time gate; intentional retirement is fine. Demotion (carrying a pattern forward at a lower level) does NOT trigger this — only complete dropout. Default `min_level=2` so 1x ("Developing") drops stay invisible (normal lifecycle). Pattern rename (old name absent, new name present) currently triggers a false-positive omission entry — documented limitation, see `OmittedPattern` docstring.
...
These gaps are documented honestly because they are reachable under adversarial-agent or drift-leaking conditions. The hostile-test results that surfaced them live at `phase1_tests/` in the flow project repo; reproducibility scripts ship with the results. **v0.3.2 adds Move #2 omission audit, Move #3 cross-session corpus-overlap demotion (per-prior reframe), and Move #4's library-layer contradiction-detection substrate; v0.4.3 moves the methodology-layer contradiction-scan instruction into the library itself (`prepare_wrap` emits it with the wrap, so the discipline can't be dropped by retiring an external doc) and extends the cross-session anti-sycophancy check onto the decoupled co-citation link path, so a sycophantic re-graduation can no longer strengthen the association graph on the ungrounded path. The operator-review layer — an LLM-as-judge pass that turns the contradiction substrate into actual semantic-opposition closure — runs separately by design (the no-LLM-as-judge axiom).** Until that operator pass runs, "immune system" describes the citation-layer structural primitives above plus the omission audit + per-prior corpus check, plus the library-layer contradiction substrate and its now-structural scan discipline — not a complete defense against every form of memory drift.
...
**anneal is the substrate; the harness fires it.** The library owns the crystallized store and the on-demand recall API — `retrieve_patterns(crystal_store, query)`, plus `anneal-memory crystal index --json` (the always-on menu of what's in the store) and `anneal-memory crystal recall <query> --json` (run-context expansion), and the same two surfaces over MCP for in-conversation agents (the `crystal_index` / `crystal_recall` tools). It does not — and cannot — fire on its own: surfacing the right pattern at the right moment requires a per-turn hook, and a hook is harness-specific (a Claude Code hook would break the 12-framework neutrality the library guarantees). So raw anneal gives you the store, the API, and *manual* recall — across library, CLI, and MCP you query if you remember to, which is the dead-store failure mode that discipline always rots into. A harness with hooks runs that recall on every prompt and surfaces the relevant crystallized patterns on cue — the *check* becomes an invariant instead of a discipline you remember (or forget). **flow** does this today; **[Levain](https://github.com/levainhq/levain)** fires the prospective (spore) layer today, with per-turn crystallized recall landing as its v2 opening slice. The boundary is deliberate: **the store is universal; the firing is the harness's job.** This is also exactly why anneal stays zero-dep and framework-neutral while a harness can be opinionated on top of it.
