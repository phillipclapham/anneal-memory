...
- **Contradiction with existing graduated patterns.** No semantic comparison runs between candidate graduations and existing Proven-tier patterns at the library layer — and structurally cannot, under the no-LLM-as-judge axiom. **The v0.3.2 architectural response splits the responsibility across three layers:** (1) library = audit substrate — `prepare_wrap()` returns `uncovered_proven_to_check: list[str]` (all existing Proven names), `extract_contradiction_declarations()` recognizes per-pattern `[contradicts: name]` / `[no-contradicts]` annotations, `detect_proven_without_declaration()` surfaces new Provens that landed without explicit stance as `proven_without_contradicts_declaration` on the save result and the hash-chained audit-log; (2) methodology layer (paired Levain WRAP_PROTOCOL.md update, shipping separately) — mandatory contradiction-scan step at graduation boundary using the library's uncovered-list; (3) operator-review (Diogenes weekly sweep, scheduled separately) — LLM-as-judge contradiction-detection pass on Proven pairs, surfacing semantic-opposition candidates for partnership review. The library layer ships in v0.3.2; the methodology and operator-review layers ship as paired follow-ups. Until all three are in place, "this gap is closed" is overclaim — the library's audit signal records discipline-followed vs not, but external review is what catches the contradictions themselves.
...
These gaps are documented honestly because they are reachable under adversarial-agent or drift-leaking conditions. The hostile-test results that surfaced them live at `phase1_tests/` in the flow project repo; reproducibility scripts ship with the results. **v0.3.2 adds Move #2 omission audit, Move #3 cross-session corpus-overlap demotion (per-prior reframe), and Move #4's library-layer contradiction-detection substrate. The methodology layer (Levain WRAP_PROTOCOL.md) and operator-review layer (Diogenes weekly sweep) that turn the contradiction-detection substrate into actual semantic-opposition closure ship as paired follow-up releases.** Until those land, "immune system" describes the citation-layer structural primitives above plus the omission audit + per-prior corpus check, plus the library-layer contradiction substrate — not a complete defense against every form of memory drift.
