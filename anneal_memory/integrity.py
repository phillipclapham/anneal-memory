...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Check who owns the wrap before cancelling it. One server process "
            "runs per client session against a shared store, so the wrap may "
            "belong to another session that is still running and part-way "
            "through composing its compression; cancelling makes that session's "
            "save fail and throws its work away. Call `status` first — it "
            "reports when the wrap started, and one opened moments ago is "
            "probably a live peer rather than a corpse. Prefer finishing a wrap "
