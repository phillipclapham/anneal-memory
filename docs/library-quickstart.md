...
The library raises errors in two categories. **`AnnealMemoryError` and everything under it** — the store-failure family (`StoreError` → `StoreDatabaseError`) and the single-writer lifecycle refusal `WrapInProgressError` (a *sibling* of `StoreError`, not a subclass — see the hierarchy below) — catch the base class at your outermost boundary, log, and let it propagate. Branching per subclass is a minority pattern for long-running servers with retry budgets or transport authors writing their own error translation layer. **Plain `ValueError`** is raised by `validated_save_continuity` for caller-contract violations — empty or malformed continuity text, no wrap in progress (`prepare_wrap` not called, or the session already wrapped), or a wrong `wrap_token`. A `ValueError` of that kind is a bug in your wrap flow, not a runtime failure to recover from: fix the call site rather than catching it. Some `ValueError`s are refusals rather than call-site bugs, and each leaves the wrap in progress with nothing saved: the catastrophic-shrink gate (re-wrap preserving the protected sections, or pass `allow_shrink=True` for a deliberate diet), and the AM-LINKGATE block, whose message starts `AM-LINKGATE refused` and means the store's association write recorded none of the pairs the wrap offered, a store defect that no change to the text clears (pass `allow_unlinked=True` only to save anyway with no links recorded), and the consolidate-gate refusal (a `session_id` that no longer holds the baton, or under `allow_sole_live` is no longer authorized; or a save whose `session_id` is not the one the wrap was prepared with; the operator decides who holds the baton, so re-claim nothing yourself — cancel the wrap or hand it back).
...
- **`WrapOwnershipError`** — a sibling of `WrapInProgressError` (also NOT a `StoreError`). Raised by `Store.wrap_cancelled(expect_token=...)` when the store's current wrap token is not the one the caller claims. Where `WrapInProgressError` stops a second wrap *clobbering* the first, this stops a second session *cancelling* it. Carries `.expected` and `.actual`; **`actual is None` means no wrap is in progress at all** (the caller's wrap already finished — retry-safe) as distinct from a wrap present and owned by someone else (backing off is the right move). The comparison happens inside the same `BEGIN IMMEDIATE` transaction that clears, so it is a true compare-and-swap across connections rather than a read-compare-write with a window. Omitting `expect_token` keeps the historical behaviour — clear whatever is current — which is what an override means here; there is no separate `force` flag.
- **`WrapInProgressError`** — a **sibling** of `StoreError`, NOT a subclass: it is a single-writer precondition refusal, not a store I/O failure, so an `except StoreError` handler deliberately does not swallow it. Raised by `prepare_wrap` (and `Store.wrap_started`) when a wrap is already in progress. Carries `.started_at` (the open wrap's timestamp) and is pickle-safe. Recovery: finish the open wrap (`validated_save_continuity`) or abandon it (`store.wrap_cancelled()`, which returns a `WrapCancelReceipt` describing what it cleared; pass `expect_token=` to abandon it only if that wrap is still the one in progress), then prepare again — or pass `allow_restart=True` to `wrap_started` to deliberately discard it. (Out-of-process callers reach the same two paths as the `save_continuity` / `wrap_cancel` MCP tools and the `save-continuity` / `wrap-cancel` CLI subcommands; the error message names all three.)
