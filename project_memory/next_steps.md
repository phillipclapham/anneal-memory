...
> ⏸ **2026-09-25 (0925+1 anneal): 0.9.15 STAGED, BUILT, twine check PASSED, NOT UPLOADED, NOT PUSHED — AWAITS PHILL'S GO (relayed by the identity head).** ALL residue from the 0.9.14 L3 is folded into one release: gate before the empty-window cancel + observe-then-CAS cancel; strict-match gated save (saver's session_id must equal the preparer's, and its wrap_token must be passed); `wrap_gated_session` key; gate re-run before `wrap_started`; cancel/compost/policy audit fixes; new public `SaveAuthorityError(ValueError)`. The tree is stamped 0.9.15 across several commits; the release commit is the last one before the dev bump (`git log --oneline -3`), and the wheel/sdist in `dist/` are built from it. **Main is stamped 0.9.15 and the pre-push gate (spore-710) blocks its push.** Sequence: upload → bump main to `0.9.16.dev0` + commit → push main → `git tag v0.9.15 <the release commit> && git push origin v0.9.15` → `gh release create v0.9.15` (wheel+sdist, notes from CHANGELOG) → verify simple index + clean-venv install. ⚠ flow must NOT re-pin to 0.9.15 until anneal_dualwrite.py uses `isinstance(e, SaveAuthorityError)` in `_BATON_REFUSALS` and its /clear advice (identity head owns it).
