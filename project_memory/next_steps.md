...
> ### 🔬 DIOGENES — NEWEST: `diogenes_20260914.md` · **STILL OPEN: 1** @ `42ae8de`
...
> ⚙ COORDINATES: 1/1 confirmed at HEAD 42ae8de. All confirmed.
> *(Pointer written 2026-09-14 by route_diogenes.py. `spore-473`: a routed report with no reader is a disposal chute.)*
...
- The audit-file READ opens left outside the helper (expect only the helper's `os.open` and the directory fd for fsync): `grep -nE "os\.open\(|gzip\.open\(|read_bytes\(|[^_a-zA-Z.]open\(" anneal_memory/audit.py anneal_memory/cli.py`. Writes (append, truncate, temp files) are out of scope; a FIFO as the active file still blocks `log()`'s append.
