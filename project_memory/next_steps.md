...
> ⏸ **2026-09-25 (0925+1 anneal): 0.9.15 STAMPED `63514b5`, BUILT, twine check PASSED, NOT UPLOADED — AWAITS PHILL'S GO (relayed via the identity head).** Closes the 3 L3 gate holes from 0.9.14 (`bcb18bd`, `e8a248c`): empty-window cancel now gated (incl. sessionless caller vs a gated wrap), a save omitting `session_id` from a gated wrap refused (new `Store.wrap_gated_session()`, `wrap_started(gated_session_id=)`), gate re-run before `wrap_started`. 2110 tests. **NOT PUSHED: main is stamped 0.9.15 and the pre-push gate (spore-710) blocks it.** Sequence: upload → bump main to `0.9.16.dev0` + commit → push main → `git tag v0.9.15 63514b5 && git push origin v0.9.15` → `gh release create v0.9.15` (wheel+sdist, notes from CHANGELOG) → verify simple index + clean-venv install.
> ⚠ **Round-2 L3 residue, deliberately NOT fixed, for Phill's ruling:** (1) codex HIGH/MED: baton take between the final authority check and commit / before `wrap_started` — the documented sidecar-outside-transaction residual; closing it means holding the baton lock across the commit (tried and withdrawn in 0.9.13 L3) or moving authority into SQLite. (2) codex HIGH: empty-path read-then-`wrap_cancelled()` TOCTOU (another session can start a wrap between the read and the cancel; fix = conditional cancel on the observed token). (3) glm HIGH, REFUTED as design: an authorized holder naming itself may cancel a wrap another session prepared, which is the stuck-wrap recovery. (4) LOW: `wrap_cancelled` audit/receipt/`had_any` do not record `wrap_gated_session`. (5) LOW, unrelated and pre-existing: compost of a zero-edge pattern writes no audit; `set_consolidate_requires_baton` audits `was` outside the transaction. (6) Design question: the gated session is only checked for being SET, not compared with the saver, so a new baton holder can commit a revoked holder's wrap.
