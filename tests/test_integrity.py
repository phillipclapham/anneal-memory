...
class TestTheTwoManifestsCannotDrift:
    """Two byte-identical files kept in sync by DISCIPLINE, and nothing else.

    ⛔ FOUND 2026-09-04 BY TWO LANES INDEPENDENTLY, from disjoint file spans —
    which is why it is here rather than in someone's notes.

    THE SETUP: ``tool-integrity.json`` exists twice, at the repo root and
    inside the package, byte-identical. Every regeneration path writes ONLY
    the package copy — ``server.py`` and ``cli.py`` both build it as
    ``Path(__file__).parent / "tool-integrity.json"`` — and
    ``test_shipped_manifest_verifies`` guards ONLY the package copy. Nothing
    writes, checks, or reads the root one. Their git histories are identical
    because a human has regenerated both every time, and the CHANGELOG says so
    in as many words on three separate releases ("both the shipped and
    repo-root manifests are regenerated"). That is a convention, not an
    invariant, and conventions are exactly what this repo has ruled it will
    not rely on.

    ⚠ AND THE CONSEQUENCE IS NOT A STALE FILE, IT IS A FALSE ALARM IN THE
    TAMPER-EVIDENCE FEATURE. Measured on this repo 2026-09-03 across the
    published-vs-repo axis of the same class: divergent manifests made
    ``verify_integrity`` return, verbatim, ``(False, ["Tool wrap_cancel
    description hash mismatch (possible tampering)"])`` — a TAMPERING verdict
    on a clean install. The root copy also ships in the sdist, so an adopter
    who takes their reference hashes from it and checks an installed package
    is the person this bites.

    Byte-identity rather than semantic equality on purpose: they are supposed
    to be the same FILE, and comparing parsed contents would pass on two files
    that differ in ways a reader would notice.
    """

    def test_the_root_manifest_is_byte_identical_to_the_shipped_one(self):
        import anneal_memory

        pkg = Path(anneal_memory.__file__).parent
        root = pkg.parent

        shipped = pkg / "tool-integrity.json"
        duplicate = root / "tool-integrity.json"

        if not duplicate.exists():
            pytest.skip(
                "no repo-root copy — this runs from a source checkout, and an "
                "installed package correctly has only the shipped manifest"
            )

        assert duplicate.read_bytes() == shipped.read_bytes(), (
            "the repo-root tool-integrity.json has drifted from the shipped "
            "one. Only the package copy is regenerated by --generate-integrity "
            "and only it is verified, so the root copy goes stale silently — "
            "and it is the copy that ships in the sdist. Divergent manifests "
            "make verify_integrity report TAMPERING on a clean install. "
            "Regenerate and copy: "
            "anneal-memory --generate-integrity && "
            "cp anneal_memory/tool-integrity.json tool-integrity.json"
        )
